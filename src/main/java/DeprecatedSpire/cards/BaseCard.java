...
import DeprecatedSpire.DeprecatedSpire;
import DeprecatedSpire.util.CardStats;
import DeprecatedSpire.util.TriFunction;
...
import static DeprecatedSpire.util.GeneralUtils.removePrefix;
import static DeprecatedSpire.util.TextureLoader.getCardTextureString;


public abstract class BaseCard extends CustomCard {
    final private static Map<String, DynamicVariable> customVars = new HashMap<>();

    protected static String makeID(String name) { return DeprecatedSpire.makeID(name); }
    ...
    public void upgrade()
    {
        if (!upgraded)
        {
            ...
            if (this.upgradesDescription)
            {
                ...
                {
                    DeprecatedSpire.logger.error("Card " + cardID + " upgrades description and has null upgrade description.");
                }
                else
                {
